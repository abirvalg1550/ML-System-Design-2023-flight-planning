...
## Дизайн ML системы - телеграм-бот "Когда покупать билеты выгодно?" Версия 0
...
- Бизнес-цель `Сделать сервис (в виде телеграм-бота), который на введенные дату/даты поездки и направление перелета покажет тенденцию к возрастанию цены в зависимости от даьы покупки билета.`
- Почему станет лучше, чем сейчас, от использования ML `Сейчас процес ценообразования на билеты скрыт от покупателя.`
`Существует эвристика в виде "Лучше покупать билеты за ~2 месяца до вылета."`
`Многие люди не могут сильно заранее планировать свои путешествия, этот бот поможет им выявить риски дополнительных трат при откладывании планирования отпуска/покупки билетов на самолет. `
`Почему ML ?  - цена на билеты зависит от спроса на конкретное направление сложным образом сильно зависит от спроса и политики ценообразования авиакомпании.`
`Простыми бизнес-правилами описать эти факторы одновременно для нескольких авиакомпаний кажется невозможным. `
`Попытаться выявить сезонные факторы для направлений с помощью анализа данных кажется более реальным.`
- Что будем считать успехом итерации с точки зрения бизнеса `Успехом будет считаться, если:`
  - ` 1) R2 > 0.8 (будет объяснять более 80% дисперсии цены на 1 км дооги в рублях) https://scikit-learn.org/stable/modules/generated/sklearn.metrics.r2_score.html) `
  - ` 2) прогноз отправляеься пользователю не позже, чем через 5 секунд после запроса прогноза;`
  - ` 3) при наступлении предполагаемой даты покупки билетов - пользователы высылается отчет о точности прогноза;`
